...
# 项目安装路径
install_path=$INTEL_CAFFE_INSTALL_PATH
...
pushd $SOURCE_ROOT/$INTEL_CAFFE_FOLDER
...
$CMAKE_EXE "$(dirs +1)" $CMAKE_VARS_DEFINE -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX=$install_path \
  ...
  -DDISABLE_BN_FOLDING=OFF \
  -DISABLE_CONV_RELU_FUSION=OFF \
  -DISABLE_BN_RELU_FUSION=OFF \
  -DISABLE_CONV_SUM_FUSION=OFF \
  -DISABLE_SPARSE=OFF \
  -DUSE_MLSL=OFF \
	-DBLAS=mkl \
	-DUSE_OPENCV=on  \
